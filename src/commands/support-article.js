...
module.exports = {
	...
	async execute(interaction) {
		...
		await interaction.editReply({
			embeds: [
				new EmbedMaker(interaction.client).setTitle(article.title).setFields(
					{
						name: 'Link',
	...
	async autocomplete(interaction) {
		...
		interaction.respond(
			articles
				.filter(
					(article) => article.name.toLowerCase().includes(search) || article.title.toLowerCase().includes(search),
				)
				.slice(0, 25)
				.map((article) => ({
					name: article.name,
