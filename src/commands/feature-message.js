...
module.exports = {
	...
	async execute(interaction) {
		...
		if (home.data.channel === interaction.targetMessage.channelId || home.data.messages.includes(interaction.targetId))
			return interaction.editReply(localize(locale, 'FEATURE_MESSAGE_ALREADY_SET'));
			...
			components: [
				new ActionRowBuilder().setComponents(
					new StringSelectMenuBuilder()
						...
						.setOptions(
							new StringSelectMenuOptionBuilder().setLabel(localize(locale, 'FOR_24_HOURS')).setValue('86400000'),
							new StringSelectMenuOptionBuilder().setLabel(localize(locale, 'FOR_3_DAYS')).setValue('259200000'),
							new StringSelectMenuOptionBuilder().setLabel(localize(locale, 'FOR_7_DAYS')).setValue('604800000'),
						),
				),
			],
		});
