	...
	async execute(interaction) {
		...
		if (subcommand === 'list') {
			interaction
				.editReply({
					embeds: [
						new EmbedMaker(interaction.client).setTitle(localize(locale, 'MESSAGE_REMINDERS')).setDescription(
							reminders.length > 0
								? `${reminders
										.slice(0, 30)
										.map(
											(reminder, index) =>
												`${index + 1}. Ends in <t:${reminder.time}:R>\n\`\`\`${
													reminder.content === ''
														? localize(locale, 'NO_CONTENT')
														: reminder.content.length > 100
														? `${reminder.content.slice(0, 100)}...`
														: reminder.content
												}\`\`\``,
										)
										.join('\n')}${reminders.length > 30 ? `\n**(${reminders.length - 30} more)**` : ''}`
								: localize(locale, 'NO_MESSAGE_REMINDERS'),
						),
					],
				})
		...
		autocomplete.respond(
			reminders.map((reminder, index) => ({
				name: `${index + 1}. ${
					reminder.content === ''
						? 'No content'
						: reminder.content.length > 25
						? `${reminder.content.slice(0, 25)}...`
						: reminder.content
				}`,
				value: `${index + 1}`,
			})),
