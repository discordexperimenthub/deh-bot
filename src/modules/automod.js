...
module.exports = class AutoMod {
    ...
    async check(message, rawContent = false) {
        let history = rawContent ? [] : message.channel.messages.cache.toJSON().slice(-11).map(m => `"${m.content}"`);
