...
module.exports = class AutoMod {
	...
	async ai(message, rawContent = false) {
		...
		let response = (
			await axios
				.post(
					`https://beta.purgpt.xyz/${this.data.ai.model.owner}/chat/completions`,
					{
		...
		if (!response || !response?.choices?.[0]?.message?.content)
			return logger(
				'error',
				'AUTOMOD',
				'Failed to get response from PurGPT API.',
				JSON.stringify(response?.error?.message ? { message: response?.error?.message } : response, null, 4),
			);
		...
		if (data.deleteMessage || data.warnMessage) {
			if (!this.data.ai.rules[data.rule - 1])
				return logger('error', 'AUTOMOD', 'Failed to get rule:', data.rule, this.data.ai.rules);
			...
			logger(
				...
				this.data.ai.rules[data.rule - 1],
				sendData,
			...
			if (this.data.ai.alertChannel) {
				...
				await channel.send({
					content: `**AutoMod** has ${data.deleteMessage ? 'blocked' : 'warned'} a message from <@${
						message.author.id
						...
						new EmbedMaker(message.client)
							...
							.setFields(
									  ...
									  ]),
								{
									name: 'Warning',
									value:
										'AutoMod is an **experimental** feature. If you think this is a mistake, please report this issue in [our Discord server](https://discord.gg/experiments).',
								},
							),
					],
				});
	...
	async badContent(message, rawContent = false) {
		...
		try {
			response = await axios.post(
				'https://beta.purgpt.xyz/openai/moderations',
				{
		...
		if (!response || response.status !== 200)
			return logger(
				...
				'Failed to get response from PurGPT API.',
				JSON.stringify(
					response?.data?.error?.message ? { message: response?.data?.error?.message } : response,
					null,
					4,
				),
			);
		...
		if (triggered) {
			...
			await message.reply(
				`Your message has been deleted by AutoMod because it is against the server rules.\n**Reason:** ${triggers
					.map((trigger) => localize('en-US', trigger.toUpperCase().replaceAll('-', '_').replaceAll('/', '_')))
	...
	async toxicContent(message, rawContent = false) {
		...
		try {
			response = await axios
				.post(
					'https://beta.purgpt.xyz/google/moderations',
					{
		...
		if (!response || response.status !== 200)
			return logger(
				'error',
				'AUTOMOD',
				'Failed to get toxic content filter response from PurGPT API.',
				JSON.stringify(
					response?.data?.error?.message ? { message: response?.data?.error?.message } : response,
					null,
					4,
	...
	async removeBlacklistChannels(category, channels) {
		this.data[category].channelBlacklist = this.data[category].channelBlacklist.filter(
