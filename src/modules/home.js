...
module.exports = class Home {
	...
	async send(featured, channel, message, time = 43200000 * 2) {
		...
		let m = `${emojis.featuredMessage} ${
			featured ? `**[Featured Post](${messageLink})**` : `**[Original Message](${messageLink})**`
		}\n${homeMessage.content}\n${message.data.replies
			.slice(0, 3)
			.map(
				(reply, index) =>
					`${message.data.replies.length - 1 > index ? emojis.replyContinuing : emojis.reply} **<@${reply.author}>:** ${
						reply.content
					}`,
			)
			.join('\n')}`;
		...
		try {
			...
			post = await webhook.send({
				avatarURL: homeMessage.member
					? homeMessage.member.displayAvatarURL({ forceStatic: true })
					: homeMessage.author.displayAvatarURL({ forceStatic: true }),
