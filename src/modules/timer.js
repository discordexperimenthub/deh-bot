...
async function sendUserMessage(userId, message) {
	...
	return (
		await axios
			.post(`https://canary.discord.com/api/v10/channels/${channel.id}/messages`, message, {
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bot ${process.env.DISCORD_TOKEN}`,
				},
...
async function deleteMessage(channelId, messageId) {
	return (
		await axios
			.delete(`https://canary.discord.com/api/v10/channels/${channelId}/messages/${messageId}`, {
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bot ${process.env.DISCORD_TOKEN}`,
				},
