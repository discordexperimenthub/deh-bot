	...
	interaction.editReply({
				...
				.setFields(
					{
						...
						value:
							automod.data.toxicContent.filters === 'all'
								? localize(locale, 'ALL')
								: automod.data.toxicContent.filters.length > 0
								? automod.data.toxicContent.filters
										.map((filter) => localize(locale, filter.toUpperCase().replaceAll('-', '_').replaceAll('/', '_')))
										.join(', ')
								: localize(locale, 'NONE'),
						inline: false,
					...
					{
						name: localize(locale, 'ALERT_CHANNEL'),
						value: automod.data.toxicContent.alertChannel
							? `<#${automod.data.toxicContent.alertChannel}>`
							: localize(locale, 'NONE'),
						inline: true,
					},
					...
					},
					{
						name: localize(locale, 'BLACKLISTED_ROLES'),
						value:
							automod.data.toxicContent.roleBlacklist.length > 0
								? automod.data.toxicContent.roleBlacklist.map((role) => `<@&${role}>`).join(', ')
								: localize(locale, 'NONE'),
						inline: false,
					},
					{
						name: localize(locale, 'BLACKLISTED_CHANNELS'),
						value:
							automod.data.toxicContent.channelBlacklist.length > 0
								? automod.data.toxicContent.channelBlacklist.map((channel) => `<#${channel}>`).join(', ')
								: localize(locale, 'NONE'),
						inline: false,
					},
		...
		components: [
			...
			new ActionRowBuilder().setComponents(
				...
				new ButtonBuilder()
					.setCustomId(
						`${interaction.user.id}:automod_${automod.data.purgptKey ? 'toxic_content_model_key' : 'ai_model'}`,
					)
					.setEmoji(emojis.aiModel.split(':')[2].replace('>', ''))
					.setLabel(localize(locale, 'AI_MODEL'))
					.setStyle(ButtonStyle.Secondary),
